...
export const Mybookingcomponent = ({booking,organizer}) => {
  ...
  const handleupdate =async(e)=>{
    ...
    try {
      const response = await fetch(`http://localhost:5000/booking/update/${bookingId}`,{
        method:'PATCH',
  ...
  const handleDelete =async()=>{
    try {
      const response = await fetch(`http://localhost:5000/booking/delete/${bookingId}`,{
        method: 'DELETE',
      });
      if (!response.ok) {
    ...
    <>
       <div className="booking-card">
          ...
          <div className="booking-details">
                ...
                <p><strong>PAN No:</strong> {booking.panno}</p>
                <p><strong>Booking Status:</strong> {booking.bookingstatus ? 'Confirmed' : 'Pending'}</p>
          ...
          <div className="btn-section">
              <Link className='cancel-btn'onClick={()=>handleDelete()}> Cancel Booking</Link>
              <Link className='update-btn' onClick={()=>toggleform()}>Update Details</Link>
          </div>
        <>{updateform &&(
          <div className="formupdate">
            ...
            <form  onSubmit={handleupdate}>
              ...
              <input type="number"  onChange={(e)=>setpin(e.target.value)} value={pin}/>
              <label htmlFor="district">District</label>
              <input type="text"  onChange={(e)=>setdistrict(e.target.value)} value={district} />
              <label htmlFor="contact">Contact</label>
              <input type="tel"  onChange={(e)=>setcontact(e.target.value)} value={contact}/>
